...
<body>
    ...
    if (isset($_POST['registrar'])) {
        ...
        $empresa_name=trim(mysqli_real_escape_string($llave, $_POST['empresa_name']));
        ...
        if (!validarNombres($empresa_name) || !validarNombres($responsable_name) || !validarApellidos($primerApell) || empty($localidad) || !validarTelefono($num_telefono) || !filter_var($email, FILTER_VALIDATE_EMAIL) || !validarPass($pass1, $pass2)) {
            ...
            $result=siEstaRegistradoCentro('empresas', 'empresa_name', $empresa_name);
            
            if ($result!="noExiste") {//si existe muestro error
                $errorYaExisteCentro=$result;
            }
            else {
                $insertarCentro="insert into empresas values(null, '$empresa_name', '$responsable_name', '$primerApell', '$segundoApell', '$localidad', '$num_telefono', '$email')";
   ...
   ?>
    ...
    <form action="registrarEmpresa.php" method="post">
        <div class="container">
            <h2 class="display-4 text-center">Registrar Empresa</h2>
            <div class="row">
                <div class="col-md-6">
                    <input type="text" id="empresa_name" name="empresa_name" class="form-control" placeholder="Nombre del centro"
                        required="" autofocus="">
                </div>
                <div class="col-md-6">
